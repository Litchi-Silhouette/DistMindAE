...
int main(int argc, char** argv) {
    if (argc < 7) {
        ...
        cout << string("program [StrategyName] [AddrForClient] [PortForClient] [AddrForServer] [PortForServer] [AddrForCache] [PortForCache] [MetadataStorageAddr] [MetadataStoragePort]") << endl;
        return 0;
    }
    ...
    string address_for_server(argv[4]);
    int port_for_server = stoi(argv[5]);
    string address_for_cache(argv[6]);
    int port_for_cache = stoi(argv[7]);
    string metadata_storage_address(argv[8]);
    int metadata_storage_port = stoi(argv[9]);

    _metadata.reset(new MetadataManager());
    // _strategy = Strategy::getStrategy(strategy_name);
    _strategy.reset(new BasicStrategy());
...
void handleRequest(shared_ptr<LBTask> request) {
    ...
    // if (_metadata->check_idle_server() < 0) {
    //     bool train = request->model_name.find("train") != string::npos;
    //     _dispatcher->push(request, train);
    //     return;
    // }
    ...
    ServerSelection selection = _strategy->calculate(_metadata, request->model_name);
    // cout << "Strategy complete" << endl;

    if (selection.ip_int == 0) {
        bool train = request->model_name.find("train") != string::npos;
        _dispatcher->push(request, train);
        return;
    }

    uint64_t cache_id = (((uint64_t)selection.ip_int) << 32);
    if (! _metadata->check_cache(selection.ip_int, request->model_name)) {
        shared_ptr<CacheAgent> cache_agent = CacheAgent::getCacheAgent(cache_id);

        // Cache out
        size_t cache_limit = _metadata->get_cache_limit(selection.ip_int);
        while (_metadata->get_cache_used(selection.ip_int) + model_size * MEMORY_MANAGER_AMPLIFIER > cache_limit) {
            string model_out = _metadata->cache_out_model(selection.ip_int);
    ...
    _metadata->cache_in_model(selection.ip_int, request->model_name);
    // cout << "Cache In" << endl;

    uint64_t server_id = (((uint64_t)selection.ip_int) << 32) + ((uint64_t)selection.port);
