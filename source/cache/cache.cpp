...
bool onceManage() {
    if (!_queue_for_manage.empty()) {
        ...
        if (instr->op == FETCH_MODEL) {
            ...
            char* model_info = _memory_manager->getShmPointer() + instr->data->data0.offset;
            ...
            char* model_info = _memory_manager->getShmPointer() + _model_manager->get(instr->key)->data0.offset;
            char* cursor = (char*)model_info;
            int pyinfo_length = *(int*)cursor;
            cursor += (4 + pyinfo_length);
            int num_batch = *(int*)cursor; cursor += 4;
...
bool onceForLB() {
    shared_ptr<LBInstruction> lb_instr = _lb_agent->getInstruction();
    ...
    if (lb_instr->op == SIGNAL_CACHE_IN)
        _lb_agent->replySignal();
