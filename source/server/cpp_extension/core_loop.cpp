...
bool onceHandleTask() {
    if (!_queue_for_new_task.empty()) {
        ...
        if (!cached) {
            ...
            if (ret->num > 1)
                cout << "Model Info Overflow" << endl;

            _cached_info[model_name] = (char*)malloc(ret->size);
            memcpy(_cached_info[model_name], _cache_agent->getShmPointer() + ret->data0.offset, ret->size);
        }
...
bool onceCopy() {
    if (!_queue_for_param_src.empty() && !_queue_for_param_dst.empty()) {
        ...
        // cout << "Begin transfer in C++: " << src->num << endl;
        ...
        for (size_t i = 0; i < src->num; ++i) {
            ...
            auto e = cudaMemcpyAsync(ptr_dst, ptr_src, size, cudaMemcpyHostToDevice, _cuda_stream);
            // cout << (uint64_t) ptr_dst << ", " << (uint64_t) ptr_src << ", " << size << endl;
