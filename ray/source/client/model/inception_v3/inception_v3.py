...
def _make_func_list(layers):
    ...
    _make_basic_conv2d(func_list, layers, 6, -1) # x = self.Conv2d_3b_1x1(x) # 14
    _make_basic_conv2d(func_list, layers, 8, -1) # self.Conv2d_4a_3x3(x) # 17
    func_list.append(("max_pool2d", [-1], {}, {"kernel_size":3, "stride":2}, [], [])) #18

    _make_inceptionA(func_list, layers, 10) # self.Mixed_5b(x) 42
    _make_inceptionA(func_list, layers, 24) # self.Mixed_5c(x) 66
    _make_inceptionA(func_list, layers, 38) # self.Mixed_5d(x) 90
    _make_inceptionB(func_list, layers, 52) # self.Mixed_6a(x) 105
    _make_inceptionC(func_list, layers, 60) # self.Mixed_6b(x) 138
    _make_inceptionC(func_list, layers, 80) # self.Mixed_6c(x) 171 v
    _make_inceptionC(func_list, layers, 100) # self.Mixed_6d(x) 204
    _make_inceptionC(func_list, layers, 120) # self.Mixed_6e(x) 237

    # aux is None in eval model
    _make_inceptionD(func_list, layers, 140) # self.Mixed_7a(x) 258
    _make_inceptionE(func_list, layers, 152) # self.Mixed_7b(x) 292
    _make_inceptionE(func_list, layers, 170) # self.Mixed_7c(x) 326
    ...
    func_list.append(("linear", [-1], *extract_hyperparameters(layers[188][0], "linear"), [], [])) # x = self.fc(x)
...
def import_model_reimpl_with_batching(train=False, device='cuda'):
    func_list = import_model_reimpl(train, device)
    return func_list, generate_batch_basic(func_list)