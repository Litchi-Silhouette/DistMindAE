    ...
    if is_inf:
        while True:
            ...
            if 'bert_base' in model_name or 'gpt2' in model_name:
                data = torch.from_numpy(np.frombuffer(data, dtype=np.long))
            else:
...
def trainingProc(gpu_id, env, control_val: mp.Value, local_rank, world_size):
    """  
    """
    base_mps_dir = '/tmp/nvidia-mps/'
    base_log_dir = '/tmp/nvidia-log/'
    ...
    log_folder = os.path.expanduser("~/pipeps/training_logs")
