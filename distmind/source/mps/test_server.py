...
def _tcp_server_thd(qname, tcp_server, tcp_lock:thd.Lock):
...
def main():
    ...
    cs = []
    for _ in range(1):
        consumer = mp.Process(target=_proc_consumer, args=(qname,))
        consumer.start()
        cs.append(consumer)
    ...
    server_lck = mp.Lock()
    for _ in range(4):
        t = mp.Process(target=_tcp_server_thd, args=(qname, server, server_lck))
        t.start()
