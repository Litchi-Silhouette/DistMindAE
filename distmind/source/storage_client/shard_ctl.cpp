...
std::string log_dir = "/home/ubuntu/pipeps/build/resource/resnet152/";
int num_batches = 8;
...
int main(int argc, char const* argv[]) {
  ...
  if (argc < 2) {
    spdlog::error("usage: ./shard_ctl <num-clis>");
  }
  int num_clis = atoi(argv[1]);
  std::string sizes_file =
      "/home/ubuntu/pipeps/build/resource/resnet152/key_sizes.txt";
  std::unordered_map<std::string, size_t> key_sizes;
  load_sizes(sizes_file, key_sizes);

  int server_port = 9999;
  std::vector<std::string> storeIPs;
  storeIPs.push_back("172.31.2.54");
  storeIPs.push_back("172.31.12.207");
  storeIPs.push_back("172.31.1.148");
  storeIPs.push_back("172.31.13.249");
