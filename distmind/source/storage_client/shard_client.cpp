...
int main(int argc, char const* argv[]) {
  if (argc < 2) {
    spdlog::error("require client name prefix, ./shard_cli <name>");
    return -1;
  }
  base_name = std::string(argv[1]) + "-" + base_name;
  spdlog::set_level(spdlog::level::debug);
  std::string ctl_ip = "172.31.2.54";
  int ctl_port = 9999;

  std::vector<std::pair<std::string, std::string>> store_ip_ports;
  store_ip_ports.push_back(std::make_pair("172.31.2.54", "7778"));
  store_ip_ports.push_back(std::make_pair("172.31.12.207", "7778"));
  store_ip_ports.push_back(std::make_pair("172.31.1.148", "7778"));
  store_ip_ports.push_back(std::make_pair("172.31.13.249", "7778"));

  std::unordered_map<std::string, StoreCli*> storeClients;
  std::unordered_map<std::string, size_t> cliOffsets;
  init_store_clis(store_ip_ports, storeClients, cliOffsets);
...
void init_store_clis(
    ...
    std::unordered_map<std::string, StoreCli*>& clis,
    std::unordered_map<std::string, size_t>& offsets) {
  size_t shm_size = 100 * 1024 * 1024 * 1024UL;
  int i = 0;
