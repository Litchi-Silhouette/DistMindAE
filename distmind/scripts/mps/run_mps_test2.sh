...
setsid python build/bin/controller.py ./scripts/mps/controller.json False > tmp/mps/log_controller.txt 2>&1 &
PID_CONTROLLER=$!
...
setsid python source/mps/load_balancer.py --controller-address $CONTROLLER_IP --controller-port $CONTROLLER_PORT --client-port  $PORT_FOR_CLIENT --server-port  $PORT_FOR_SERVER > tmp/mps/log_LB.txt 2>&1 &
PID_LB=$!
...
setsid python source/mps/server_agent.py --lb-port $PORT_FOR_SERVER --lb-ip $IP_FOR_LOAD_BALANCER --size-list source/mps/model_sizes.txt --gpu-num $WORLD_SIZE --ctrl-ip $CONTROLLER_IP --ctrl-port $CONTROLLER_PORT_SERVER --local-ip $LOCAL_IP > tmp/mps/log_server.txt 2>&1 &
PID_SERVER=$!
...
./scripts/run_client_max_inference.sh > tmp/mps/log_client.txt
