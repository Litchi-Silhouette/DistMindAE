...
def import_data(batch_size):
    ...
    for sent in test_inputs:
        _ids = tokenizer.encode(
            sent, max_length=512, pad_to_max_length=True, add_space_before_punct_symbol=True
        )
...
def _bert_layer(func_list, layers, i):
    ...
    # --- self output layer
    func_list.append(("linear", [-1], *extract_hyperparameters(layers[base_idx+8][0], "linear"), [], [])) # self.dense(hidden_states) # 35
    func_list.append(("dropout", [-1], *extract_hyperparameters(layers[base_idx+9][0], "dropout"), [], [])) # self.dropout(hidden_states) #36
    func_list.append(("add", [-1, -5], {}, {}, [], [])) # hidden_states + input_tensor #37
    func_list.append(("layer_norm", [-1], *extract_hyperparameters(layers[base_idx+10][0], "layer_norm"), [], [])) # self.LayerNorm(hidden_states + input_tensor) #38
...
def _bert_pool(func_list, layers):
    ...
    func_list.append(("linear", [-1], *extract_hyperparameters(layers[base_idx][0], "linear"), [], [])) # pooled_output
    func_list.append(("tanh", [-1], *extract_hyperparameters(layers[base_idx+1][0], "tanh"), [], [])) # pooled_output
