...
./build/bin/storage $STORAGE_PORT $STORAGE_PORT > tmp/test5/$BATCH_SIZE/$MODEL_SEED/log_storage.txt 2>&1 &
PID_STORAGE=$!
...
sleep 1
FI_EFA_ENABLE_SHM_TRANSFER=0 ./build/bin/cache $LOCAL_IP $CACHE_PORT $METADATA_IP $METADATA_PORT $LOAD_BALANCER_IP $LOAD_BALANCER_PORT_CACHE cachecache $CACHE_SIZE $CACHE_BLOCK_SIZE > tmp/test5/$BATCH_SIZE/$MODEL_SEED/log_cache.txt 2>&1 &
PID_CACHE=$!
...
do
	...
	nohup python -X faulthandler ./build/bin/server.py $LOCAL_IP $GPU $LOCAL_IP $CACHE_PORT $LOAD_BALANCER_IP $LOAD_BALANCER_PORT_SERVER $CONTROLLER_IP $CONTROLLER_PORT_SERVER $BACKEND 1 >tmp/test5/$BATCH_SIZE/$MODEL_SEED/log_worker_$GPU.txt 2>&1 &
    PID_SERVER+=($!)
    sleep 1
done
