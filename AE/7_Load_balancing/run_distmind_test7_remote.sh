...
mkdir -p tmp/test7/$SYSTEM_TYPE
python ./source/py_utils/launch_remote.py --launch_part create_dir  --log_dir tmp/test7/$SYSTEM_TYPE --test_index 7
sleep 5

echo "Cleaning up /dev/shm..."
python ./source/py_utils/launch_remote.py --launch_part cleanup --test_index 7
sleep 5
...
cleanServer() {
    ...
    python ./source/py_utils/launch_remote.py --launch_part dist_server --test_index 7 --stop 
    sleep 5
...
cleanup() {
    ...
    python ./source/py_utils/launch_remote.py --launch_part dist_storage --test_index 7 --stop
    sleep 5
...
./build/bin/metadata_storage 0.0.0.0 $METADATA_PORT > tmp/test7/$SYSTEM_TYPE/log_metadata.txt 2>&1 &
PID_METADATA=$!

# Step 3: Storage
echo "Starting storage..."
python ./source/py_utils/launch_remote.py --launch_part dist_storage --test_index 7 --log_file tmp/test7/$SYSTEM_TYPE/$MODEL_SEED/log_storage.txt
...
run_experiment() {
    ...
    python build/bin/controller.py --config settings/controller.json --use_train False --zipf_s $zipf --rescheduling_period $resched > tmp/test7/$SYSTEM_TYPE/log_controller_zipf${zipf}_resched${resched}.txt 2>&1 &
    PID_CONTROLLER=$!
    ...
    ./build/bin/$SYSTEM_TYPE basic 0.0.0.0 $PORT_FOR_CLIENT 0.0.0.0 $PORT_FOR_CACHE 0.0.0.0 $PORT_FOR_SERVER $CONTROLLER_IP $CONTROLLER_PORT $METADATA_IP $METADATA_PORT > tmp/test7/$SYSTEM_TYPE/log_LB_zipf${zipf}_resched${resched}.txt 2>&1 &
    PID_LB=$!
    ...
    python ./source/py_utils/launch_remote.py --launch_part dist_server --test_index 7 --n_models $NUM_MODELS --model_seed $MODEL_SEED --system_type $SYSTEM_TYPE 
    ...
    ./scripts/run_client_max_inference.sh > tmp/test7/$SYSTEM_TYPE/log_client_zipf${zipf}_resched${resched}.txt
