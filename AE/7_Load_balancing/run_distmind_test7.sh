...
./build/bin/storage $STORAGE_PORT $STORAGE_PORT > tmp/test7/$SYSTEM_TYPE/log_storage.txt 2>&1 &
PID_STORAGE=$!
...
run_experiment() {
    ...
    sleep 1
    FI_EFA_ENABLE_SHM_TRANSFER=0 ./build/bin/cache $LOCAL_IP $CACHE_PORT $METADATA_IP $METADATA_PORT $LOAD_BALANCER_IP $LOAD_BALANCER_PORT_CACHE cachecache $CACHE_SIZE $CACHE_BLOCK_SIZE > tmp/test7/$SYSTEM_TYPE/log_cache_zipf${zipf}_resched${resched}.txt 2>&1 &
    PID_CACHE=$!
    ...
    do
        ...
        nohup python ./build/bin/server.py $LOCAL_IP $GPU $LOCAL_IP $CACHE_PORT $LOAD_BALANCER_IP $LOAD_BALANCER_PORT_SERVER $CONTROLLER_IP $CONTROLLER_PORT_SERVER $BACKEND $WORLD_SIZE > tmp/test7/$SYSTEM_TYPE/log_worker_${GPU}_zipf${zipf}_resched${resched}.txt 2>&1 &
        PID_SERVER+=($!)
        sleep 1
    done
